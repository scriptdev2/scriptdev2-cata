...
char remotes[NUM_REMOTES][MAX_REMOTE] =
{
    ...
    "git:// github.com/scriptdev2/scriptdev2-cata.git"      // used for fetch if present
};
...
{
    "REVISION_DB_SCRIPTDEV2",
    "REVISION_DB_SD2_MANGOS",
};
...
int get_rev(const char* from_msg)
{
    ...
    char nr_str[256];
    if (sscanf(from_msg, "[" GIT_REV_PREFIX "%[0123456789]]", nr_str) != 1) return 0;
    // ("[")+(REV_PREFIX)+("]")-1
    if (from_msg[strlen(nr_str) + strlen(GIT_REV_PREFIX) + 2 - 1] != ']') return 0;
...
std::string generateNrHeader(char const* rev_str)
{
    ...
    newData << " #define SD2_REVISION_NR \"" << rev_str << "\"" << std::endl;
...
{
    std::ostringstream newData;
    newData << "#ifndef __SD2_REVISION_SQL_H__" << std::endl;
    newData << "#define __SD2_REVISION_SQL_H__"  << std::endl;
    for (int i = 0; i < NUM_DATABASES; ++i)
    {
        newData << " #define " << db_sql_rev_field[i] << " \"required_" << last_sql_update[i] << "\"" << std::endl;
    }
    newData << "#endif // __SD2_REVISION_SQL_H__" << std::endl;
    return newData.str();
}
...
bool write_rev_sql()
{
    ...
    printf("+ writing sd2_revision_sql.h\n");
...
bool amend_commit()
{
    ...
    fprintf(cmd_pipe, GIT_REV_FORMAT " %s", rev, head_message);
    pclose(cmd_pipe);
        ...
        if (itr != new_sql_updates.end())
        {
                                 ...
                                 (info.rev == last_sql_rev[info.db_idx] && info.nr > last_sql_nr[info.db_idx])))
            {
                ...
                if (db_sql_rev_parent[info.db_idx])
                    snprintf(last_sql_update[info.db_idx], MAX_PATH, "_%s%s%s", info.db, info.has_table ? "_" : "", info.table);
                else
